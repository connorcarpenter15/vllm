//! Conversion between engine RPC v1 protobuf types and the internal
//! `vllm-text` request/response types.
//!
//! This mirrors [`crate::grpc::convert`] (the `vllm` Generate service), with
//! the metadata required by out-of-process frontends.
...
/// Convert an engine RPC `GenerateRequest` into the internal `TextRequest`.
///
...
pub fn to_text_request(
    ...
    Ok(TextRequest {
        ...
        sampling_params,
        decode_options,
        // The engine RPC Generate RPC is always server-streaming; the `stream`
        ...
        match modality {
            ...
            other => {
                return Err(Status::unimplemented(format!(
                    "media modality {other:?} is not supported by the vLLM engine RPC service \
                     (image only in v1)"
                )));
            }
        }
...
fn build_sampling_params(sampling: Option<&pb::SamplingParams>) -> SamplingParams {
    ...
    SamplingParams {
        ...
        // Seed `0` is indistinguishable from "unset" in the engine RPC proto
...
/// Map a decoded text event to the engine RPC `GenerateResponse` messages it
...
pub fn event_to_responses(
    ...
    match event {
        // engine RPC has no dedicated "start" event; prompt metadata is not
...
fn terminal_response(
    ...
    if role == pb::EngineRole::Prefill {
        let kv_session = finished
            .kv_transfer_params
            .as_ref()
            .map(|params| kv_transfer_params_to_kv_session(params, request_id, kv_connector));
        return pb::GenerateResponse {
            request_id: request_id.to_string(),
            event: Some(pb::generate_response::Event::PrefillReady(
                pb::PrefillReady { kv_session },
            )),
            usage,
        };
    }
...
/// Map the internal finish reason to the engine RPC enum.
///
/// `Repetition` (a model-driven natural stop) maps to `STOP`; engine RPC has no
...
/// Derive the engine RPC role from the engine's authoritative KV transfer role.
///
...
pub fn mark_prefill_request(request: &mut TextRequest) {
    let xargs = request.sampling_params.vllm_xargs.get_or_insert_with(Default::default);
    let entry = xargs
...
mod tests {
    ...
    fn prefill_terminal_emits_prefill_ready() {
        ...
        let attrs = prost_struct_to_json(
            session.attributes_struct.as_ref().expect("attributes_struct present"),
        );
    ...
    fn mark_prefill_sets_do_remote_decode() {
        let mut text =
            to_text_request(base_request(), &["test-model".to_string()]).expect("convert");
    ...
    fn role_mapping() {
        ...
        assert_eq!(
            role_from_kv_role(Some("kv_both")),
            pb::EngineRole::Aggregated
        );
