...
use crate::lora_path::{
    ...
    runtime_lora_allowed_path_prefixes, validate_lora_path_access,
};
...
async fn normalize_lora_adapter(adapter: pb::LoraAdapter) -> Result<LoraRequest, Status> {
    ...
    let canonical = validate_lora_path_access(
        ...
        LoraPathMode::LocalDirectory,
    )
    .await
    .map_err(lora_path_status)?
    .ok_or_else(|| Status::internal("local LoRA path validation returned a remote identifier"))?;
...
fn lora_path_status(error: LoraPathError) -> Status {
    match error {
        LoraPathError::MissingAllowedPrefixes => Status::failed_precondition(format!(
            "local LoRA paths require {RUNTIME_LORA_ALLOWED_PATH_PREFIXES_ENV}"
        )),
        LoraPathError::MustBeAbsolute => Status::invalid_argument("source_path must be absolute"),
        LoraPathError::InvalidPath { source } => Status::invalid_argument(format!(
            "invalid source_path: {}",
            source.to_report_string()
        )),
        LoraPathError::InvalidPrefix { source } => Status::internal(format!(
            "configured {RUNTIME_LORA_ALLOWED_PATH_PREFIXES_ENV} prefix is invalid: {}",
            source.to_report_string()
        )),
        LoraPathError::NotDirectory => Status::invalid_argument("source_path must be a directory"),
        LoraPathError::OutsideAllowedPrefixes => {
            Status::permission_denied("source_path is outside the configured LoRA path prefixes")
        }
    }
}
