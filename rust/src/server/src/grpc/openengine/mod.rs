//! OpenEngine v1 gRPC service backed by the shared [`vllm_text::TextLlm`]
//! facade.
//!
//! This is the vendor-neutral runtime API consumed by the Dynamo vLLM sidecar.
//! It is a sibling of the `vllm`-native [`crate::grpc::GenerateServiceImpl`] and
...
use vllm_text::{
    Prompt, SamplingParams, TextDecodeOptions, TextOutputStreamExt as _, TextRequest,
};
...
/// API version string advertised by [`OpenEngineServiceImpl::get_engine_info`].
const OPENENGINE_API_VERSION: &str = "openengine.v1";
...
/// OpenEngine v1 service implementation backed by the shared application state.
...
impl OpenEngineServiceImpl {
    ...
    /// metadata advertised over OpenEngine.
...
impl pb::open_engine_server::OpenEngine for OpenEngineServiceImpl {
    ...
    async fn get_engine_info(
        ...
        Ok(Response::new(pb::EngineInfo {
            ...
            api_version: OPENENGINE_API_VERSION.to_string(),
            role: role as i32,
    ...
    async fn health(
        ...
        checks.push(health_check("engine", engine_state, client.health_error().map(|e| e.to_string())));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if Instant::now() >= deadline {
                    warn!(in_flight, "openengine drain deadline elapsed with requests still in flight");
                ...
                sleep_until(std::cmp::min(deadline, Instant::now() + DRAIN_POLL_INTERVAL)).await;
            }
        });
...
impl OpenEngineServiceImpl {
    fn parallelism_info(&self) -> pb::ParallelismInfo {
        ...
        pb::ParallelismInfo {
            ...
            data_parallel_size: rr.data_parallel_size,
            data_parallel_rank: rr.data_parallel_rank,
    ...
    async fn run_inference_probe(&self, model: &str) -> (pb::HealthState, Option<String>) {
        ...
        let probe = TextRequest {
            request_id: format!("openengine-health-{}", Uuid::new_v4()),
