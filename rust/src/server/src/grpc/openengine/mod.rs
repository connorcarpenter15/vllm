...
impl OpenEngineService {
    ...
    fn kv_connector_info(&self) -> pb::KvConnectorInfo {
        ...
        pb::KvConnectorInfo {
            ...
            transfer_backend: enabled.then_some(TRANSFER_BACKEND).unwrap_or_default().to_string(),
            local_endpoints: Vec::new(),
            supported_protocols: enabled
                .then(|| vec![TRANSFER_BACKEND.to_string()])
                .unwrap_or_default(),
            ...
            handoff_profile: enabled.then_some(HANDOFF_PROFILE).unwrap_or_default().to_string(),
            ...
            while let Some(event) = stream.next().await {
                ...
                for response in responses {
                    if tx.send(Ok(response)).await.is_err() {
                        let _ = state.chat.abort(std::slice::from_ref(&request_id)).await;
                        return;
                    }
                }
                if terminal_error {
...
impl pb::control_server::Control for OpenEngineService {
    ...
    async fn get_load(
        ...
        let ranks = include_per_rank
            .then(|| {
                ready
                    .iter()
                    .map(|response| pb::RankLoadInfo {
                        data_parallel_rank: Some(response.data_parallel_rank),
                        running_requests: (ready.len() == 1).then_some(load),
    ...
    async fn health(
        ...
        let mut checks = vec![
            ...
            pb::HealthCheck {
                ...
                message: (!role_matches)
                    .then(|| format!("expected {expected_role:?}, serving {:?}", self.role))
                    .unwrap_or_default(),
            },
        ];
            ...
            loop {
                    ...
                    && deadline.is_some_and(|deadline| tokio::time::Instant::now() >= deadline)
                {
                    if request.abort_after_deadline {
                        if let Err(error) = state.chat.abort(&[]).await {
                            let _ = tx
...
fn drain_update(state: pb::DrainState, load: u64, message: &str) -> pb::DrainResponse {
    pb::DrainResponse {
