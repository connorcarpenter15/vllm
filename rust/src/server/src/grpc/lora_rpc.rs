...
pub(super) async fn list_loras(
    ...
    Ok(Response::new(pb::ListLorasResponse {
        adapters: adapters.iter().map(to_proto).collect(),
    }))
}
...
async fn normalize_adapter(adapter: pb::LoraAdapter) -> Result<LoraRequest, Status> {
    ...
    validate_allowed_prefix(&canonical).await?;
...
async fn validate_allowed_prefix(path: &Path) -> Result<(), Status> {
    ...
    for prefix in prefixes {
        let canonical = tokio::fs::canonicalize(prefix).await.map_err(|error| {
            Status::internal(format!(
                "configured {RUNTIME_LORA_ALLOWED_PATH_PREFIXES_ENV} prefix is invalid: {error}"
            ))
        })?;
