...
async fn post_unload(app: &mut axum::Router) -> StatusCode {
    app.call(
        Request::builder()
            .method("POST")
            .uri("/v1/unload_lora_adapter")
            .header("content-type", "application/json")
            .body(Body::from(json!({"lora_name": "adapter-a"}).to_string()))
            .expect("build unload request"),
    )
    .await
    .expect("call unload route")
    .status()
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn timed_out_load_runs_bounded_compensation() {
    let (mut app, engine_task) = test_admin_app_with_engine_script(|dealer, push| {
        boxed_test_future(async move {
            ...
            let payload = decode_value(&remove[1]).expect("decode compensation");
            let array = payload.as_array().expect("utility payload");
            assert_eq!(array[2], Value::from("remove_lora"));
            reply_utility(push, array[1].as_u64().unwrap(), true).await;
        })
    })
    .await;
    ...
    let models = app
        .call(Request::builder().uri("/v1/models").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn cancelled_load_marks_registry_inconsistent() {
    ...
    let mut loading_app = app.clone();
    let load =
        tokio::spawn(async move { post_load(&mut loading_app, "org/adapter-a", false).await });
    ...
    assert_eq!(
        post_load(&mut retry_app, "org/adapter-a", false).await,
        StatusCode::INTERNAL_SERVER_ERROR
    );
    drop(app);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn failed_unload_restores_adapter() {
    let (mut app, engine_task) = test_admin_app_with_engine_script(|dealer, push| {
        boxed_test_future(async move {
            let load = receive_lora_utility(dealer, "org/adapter-a", false).await;
            reply_utility(push, load, true).await;

            let unload = recv_engine_message(dealer).await;
            let payload = decode_value(&unload[1]).expect("decode unload");
            let array = payload.as_array().expect("utility payload");
            assert_eq!(array[2], Value::from("remove_lora"));
            send_outputs(
                push,
                UtilityCallOutput {
                    ...
                    output: UtilityOutput {
                        call_id: array[1].as_u64().unwrap().into(),
            ...
            let restore = receive_lora_utility(dealer, "org/adapter-a", false).await;
            reply_utility(push, restore, true).await;
        })
    })
    .await;
